        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item menu-open">
            ...
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item">
                <a href="{{route('ServiceWebsite')}}" class="nav-link active">
                  <i class="far fa-circle nav-icon"></i>
                  <p>Service</p>
                </a>
              </li>
                    ...
                    @foreach ($getAbout as $item)                          
                      <tr>
                        ...
                        <td>{{$item->deskripsi}}</td>
                         <td style="text-align: center"><img src="{{asset('storage/about/'.$item->image)}}" alt="" width="70px"></td>
                            ...
                            <div class="modal-dialog">
                              <div class="modal-content">
                                    <div class="modal-header">
                                        <h4 class="modal-title">Edit Data About</h4>
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
                                    <form action="{{route('AboutUpdate', ['id' => $item->id])}}" method="POST" enctype="multipart/form-data"> 
                                            @csrf
                                            @method('PUT')
                                        <div class="modal-body">
                                            <div class="form-group">
                                                <label for="exampleInputEmail1">Title</label>
                                                <textarea type="text" class="form-control" id="title" placeholder="" name="vtitle" value="">{{ $item->title }}</textarea>
                                                    @error('vtitle')
                                                    <small>{{ $message }}</small>
                                                    @enderror
                                            </div>
                                            <div class="form-group">
                                                <label for="exampleInputEmail1">Title Deskripsi</label>
                                                <input type="text" class="form-control" id="title" placeholder="" name="Titledeskripsi" value="{{ $item->deskripsi1 }}">
                                                    @error('Titledeskripsi')
                                                    <small>{{ $message }}</small>
                                                    @enderror
                                            </div>
                                            <div class="form-group">
                                                <label for="exampleInputEmail1">Deskripsi</label>
                                                <textarea type="text" class="form-control" id="title" placeholder="" name="vdeskripsi" value="">{{ $item->deskripsi }}</textarea>
                                                    @error('vdeskripsi')
                                                    <small>{{ $message }}</small>
                                                    @enderror
                                            </div>
                                             <div class="form-group">
                                                  ...
                                                  <img src="{{ asset('storage/about/'.$item->image)}}" alt="Current Image" style="max-width: 200px;">
                                              </div>
